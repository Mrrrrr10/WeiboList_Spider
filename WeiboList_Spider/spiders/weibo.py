...
from ..settings import IS_SINGLE
...
class WeiboListSpider(scrapy.Spider):
    ...
    info_url = 'https://m.weibo.cn/api/container/getIndex?containerid=230283{uid}_-_INFO&lfid=230283{uid}&type=uid&value={uid}'
    follow_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_followers_-_{uid}&page={page}'
    fan_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_fans_-_{uid}&since_id={since_id}'
    ...
    def parse_user(self, response):
        ...
        if result.get('data').get('userInfo'):
            ...
            yield Request(self.info_url.format(uid=uid), callback=self.parse_info,
                          meta={'user_item': user_item, 'uid': uid})
    ...
    def parse_info(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')):
            ...
            try:
                user_item['location'] = \
                [info.get('item_content') for info in result.get('data').get('cards')[1].get('card_group') if
                 info.get('item_name') == "所在地"][0]
    ...
    def parse_follows(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and \
            ...
            uid = response.meta.get('uid')
            # 关注列表
            user_relation_item = WeiboUserRelationItem()
    ...
    def parse_fans(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and \
            ...
            # 下一页粉丝
            page = response.meta.get('page') + 1
            yield Request(self.fan_url.format(uid=uid, since_id=page),
                          callback=self.parse_fans, meta={'page': page, 'uid': uid, 'retry': 10})
